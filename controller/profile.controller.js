...
const { updateProfileValidator, changePasswordValidator } = require("../validator/profile.validator");

// 1. Get Me
const getMe = async (req, res, next) => {
  try {
    const user = await Profile.findById(req.user.id).select("-password -otp -otpTime");
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    if (email && email !== user.email) {
      ...
      if (!isMatch) return next(CustomErrorHandler.BadRequest("Noto'g'ri parol"));
...
const changePassword = async (req, res, next) => {
  try {
    ...
    if (!isMatch) return next(CustomErrorHandler.BadRequest("Hozirgi parolingiz noto'g'ri"));
...
const deleteAccaunt = async (req, res, next) => {
  try {
    ...
    if (!password) return next(CustomErrorHandler.BadRequest("Akkauntni o'chirish uchun parolni kiriting"));
